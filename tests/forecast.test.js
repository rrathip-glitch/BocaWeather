...
function makeRaw({ aifsProbability = true } = {}) {
  ...
  // AIFS: precipitation only (no probability — reproduces real behavior).
  const precipAifs = time.map((_, i) => {
    const h = i % 24;
    if (h === 15) return 0.03;
    if (h === 16) return 0.05;
    return 0;
  });
  const probAifs = aifsProbability ? precipAifs.map((p) => (p > 0 ? 50 : 8)) : new Array(48).fill(null);

  const hourly = {
    time,
    [`precipitation_${MODEL_HRRR}`]: precipHrrr,
    [`precipitation_probability_${MODEL_HRRR}`]: probHrrr,
    [`precipitation_${MODEL_AIFS}`]: precipAifs,
    [`precipitation_probability_${MODEL_AIFS}`]: probAifs,
    [`temperature_2m_${MODEL_HRRR}`]: time.map(() => 80),
    [`temperature_2m_${MODEL_AIFS}`]: time.map(() => 80),
    [`weathercode_${MODEL_HRRR}`]: time.map(() => 0),
    [`weathercode_${MODEL_AIFS}`]: time.map(() => 0),
    [`windspeed_10m_${MODEL_HRRR}`]: time.map(() => 8),
    [`windspeed_10m_${MODEL_AIFS}`]: time.map(() => 9),
    [`wind_gusts_10m_${MODEL_HRRR}`]: time.map(() => 15),
    [`wind_gusts_10m_${MODEL_AIFS}`]: time.map(() => 16)
  };
  ...
  const daily = {
    ...
    [`precipitation_sum_${MODEL_AIFS}`]: [0.08, 0.08],
    [`precipitation_probability_max_${MODEL_HRRR}`]: [55, 55],
    [`precipitation_probability_max_${MODEL_AIFS}`]: [50, 50],
    [`temperature_2m_max_${MODEL_HRRR}`]: [85, 85],
    [`temperature_2m_max_${MODEL_AIFS}`]: [85, 85],
    [`temperature_2m_min_${MODEL_HRRR}`]: [70, 70],
    [`temperature_2m_min_${MODEL_AIFS}`]: [70, 70],
...
test("BUG REPRO: AIFS probability null but precip non-null → probability is derived (not null)", () => {
  const raw = makeRaw({ aifsProbability: false });
  ...
  const f = buildForecast({ data: raw, cached: false, models: { hrrr: { available: true, id: MODEL_HRRR }, aifs: { available: true, id: MODEL_AIFS } } }, { now });

  // The two storm hours (15:00, 16:00) should have AIFS probability derived,
  ...
  assert.notEqual(
    storm15.rain_probability_aifs,
    null,
    "AIFS probability must be derived from precipitation when missing"
  );
  assert.equal(storm15.rain_probability_aifs, 40); // 25 + 0.03*500
  assert.equal(storm16.rain_probability_aifs, 50); // 25 + 0.05*500

  // Dry hours stay at 0 (not null).
  const dry = f.hourly.find((h) => h.time.endsWith("T03:00"));
  assert.equal(dry.rain_probability_aifs, 0);
});

test("AIFS probability present → derivation does NOT overwrite native values", () => {
  const raw = makeRaw({ aifsProbability: true });
  const now = new Date("2026-05-19T13:00:00-04:00");
  const f = buildForecast({ data: raw, cached: false, models: { hrrr: { available: true, id: MODEL_HRRR }, aifs: { available: true, id: MODEL_AIFS } } }, { now });

  const storm15 = f.hourly.find((h) => h.time.endsWith("T15:00"));
  // Native AIFS prob in fixture was 50 (set via `p > 0 ? 50 : 8`).
  assert.equal(storm15.rain_probability_aifs, 50);
});
