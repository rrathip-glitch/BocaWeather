...
function dailyVerdictFromTennisStats({
  ...
  if (heavyHours >= NOGO_HEAVY_HOURS) {
    return {
      verdict: "NO_GO",
      reason: `Skip it: ${heavyHours} tennis hours with ≥${NOGO_HEAVY_PROB}% rain chance.`
    };
  }
  if (tennisPrecip >= NOGO_PRECIP_IN) {
    return {
      verdict: "NO_GO",
      reason: `Skip it: ${tennisPrecip.toFixed(2)}" of rain expected during tennis hours.`
    };
  }
  if (peakTennisProb >= CAUTION_PEAK_PROB || tennisPrecip >= CAUTION_PRECIP_IN) {
    return {
      verdict: "CAUTION",
      reason: `Maybe: peak ${peakTennisProb}% rain chance during tennis hours — watch the radar.`
    };
  }
  if (disagreementAtRiskyHour) {
    return {
      verdict: "CAUTION",
      reason: "Maybe: models disagree on afternoon storms — check radar 60 min before."
    };
  }
  return {
    verdict: "GO",
...
export function buildForecast({ data, cached, models }) {
  ...
  // Tennis hours (6am-9pm inclusive) used for daily verdict & headline stats.
  const tennisHours = tomorrowHours.filter(
    (h) =>
      h.is_tomorrow === true &&
      h._hour != null &&
