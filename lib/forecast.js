...
function pickBestWindow(windows) {
  const gos = windows.filter((w) => w.verdict === VERDICT.GO);
  if (gos.length > 0) {
  ...
  const lights = windows.filter((w) => w.verdict === VERDICT.LIGHT_CAUTION);
  if (lights.length > 0) {
...
export function buildForecast({ data, cached, models }) {
  ...
  const best_window = pickBestWindow(tennis_windows);
