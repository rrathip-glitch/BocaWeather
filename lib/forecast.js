...
function buildHourly(raw, now) {
  ...
  const gustAifs = h[`wind_gusts_10m_${MODEL_AIFS}`] || [];

  const todayLocal = localDateString(now);

  return times.map((t, i) => {
    let ph = num(probHrrr[i]);
    let pa = num(probAifs[i]);

    const rh = num(precipHrrr[i]);
    const ra = num(precipAifs[i]);

    // Open-Meteo's ECMWF AIFS deterministic model (`ecmwf_aifs025`) does not
    // expose `precipitation_probability` — only the AIFS ensemble does. When
