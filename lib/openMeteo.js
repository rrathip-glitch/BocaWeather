...
const HOURLY_VARS = [
  ...
  // ECMWF IFS both expose wind_gusts_10m on Open-Meteo, so it round-trips
...
async function fetchCombined() {
  const url = buildUrl([MODEL_HRRR, MODEL_IFS]);
  const res = await fetch(url);
  if (!res.ok) {
    const body = await res.text();
 ...
 * of the requested models (real observed behavior with `ecmwf_ifs025` in
 * the combined `gfs_hrrr,ecmwf_ifs025` call). Treat that case as "model
...
/**
 * Returns { data, cached, models: { hrrr: { available, id, error? }, ifs: { ... } } }
 *
 * Strategy:
 *   1. Try the combined request (one HTTP call).
 *   2. Verify each requested model actually appears in the response — Open-Meteo
 *      sometimes returns 200 OK while silently dropping IFS variables, which
 *      previously caused the UI to show an empty IFS chart line. For any
...
export async function fetchForecast({ bypassCache = false } = {}) {
  ...
  const modelsMeta = {
    ...
    ifs: { available: false, id: MODEL_IFS }
  };
  ...
  try {
    ...
    const [hrrrResult, ifsResult] = await Promise.allSettled([
      fetchModel(MODEL_HRRR),
      fetchModel(MODEL_IFS)
    ]);
    ...
    if (ifsResult.status === "fulfilled") {
      modelsMeta.ifs.available = true;
      mergeModelResponse(merged, ifsResult.value, MODEL_IFS);
    } else {
      modelsMeta.ifs.error = String(ifsResult.reason?.message || ifsResult.reason);
    }

    if (!modelsMeta.hrrr.available && !modelsMeta.ifs.available) {
  ...
  if (combinedSucceeded) {
    ...
    modelsMeta.ifs.available = hasModelData(combined, MODEL_IFS);

    // Repair: for any requested model the combined call dropped, retry it
    // alone. This recovers the IFS line in the common Open-Meteo case where
    ...
    if (!modelsMeta.ifs.available) missingModels.push({ id: MODEL_IFS, meta: modelsMeta.ifs });
